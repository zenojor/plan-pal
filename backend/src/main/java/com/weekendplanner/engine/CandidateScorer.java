...
public class CandidateScorer {
    ...
    public double score(PoiDto poi, String phase, PlanIntent intent) {
        return score(poi, phase, intent, null);
    }

    public double score(PoiDto poi, String phase, PlanIntent intent, WeatherSnapshot weather) {
        ...
        score += weatherScore(tags, weather);
    ...
    private double weatherScore(Set<String> tags, WeatherSnapshot weather) {
        if (weather == null) return 0.0;
        double score = 0.0;
        boolean high = "HIGH".equalsIgnoreCase(weather.outdoorRiskLevel());
        boolean medium = "MEDIUM".equalsIgnoreCase(weather.outdoorRiskLevel());
        if (!high && !medium) return score;
        score += match(tags, weather.preferredTags().toArray(String[]::new)) * (high ? 35 : 18);
        score -= match(tags, weather.avoidTags().toArray(String[]::new)) * (high ? 55 : 22);
        if (high) {
            score += match(tags, "indoor", "sheltered", "mall", "museum", "cafe") * 25;
            score -= match(tags, "outdoor", "citywalk", "sports") * 45;
        }
        return score;
    }
